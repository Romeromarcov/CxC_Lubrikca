...
|---|---|---|---|
| 1 | Reparto del exceso de los pagos sobreaplicados | 1.269,25 USD de exceso; los dos repartos distribuyen 2.483,00 exactos | Con «por orden», 7 órdenes pierden todo el crédito y vuelven enteras a la CxC. Cuál de los dos es una decisión de negocio. **Ojo con la cifra:** el hallazgo original decía 1.333,85 porque pasaba por la conversión a equivalente de la app; medido contra el campo crudo son 1.269,25, y es el mismo hallazgo. |
| 2 | Alcance del descuento de línea | entre −107,42 y −8.719,06 USD | Tu descripción autoriza los obsequios con precio 0 o 99 % de descuento; los otros 8.611,64 son líneas en cero *sin* esa marca. |
...
| 4 | Cuál de las **dos fuentes de configuración de listas** manda | decide con qué lista se valora cada teórico | Cinco sitios leen de dos fuentes que nada sincroniza: el mapeo unificado da USD=11/BCV=10 (activas) y las claves `valid_pricelists_*` dan USD=4/BCV=5 (archivadas, y la 5 vencida el 2-sep). Unificarlas mueve el teórico. **Medido en QA, donde esas claves no existen: el número puede no ser el de producción, pero la estructura sí.** Chequeo diario en ALTA. |
