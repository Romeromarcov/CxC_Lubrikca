...
| 2.4 | Sacar caminos de dinero de app.py | **empezada** — **diez piezas** extraídas en seis módulos, cada una con su medición A/B: `engine/saldos.py` (cuánto falta cobrar), `engine/universo.py` (qué órdenes entran), `engine/listas.py` (cuál lista valora el teórico — de ahí salió el hallazgo de las 789 órdenes), `engine/balance.py` (las 15 partidas internas **y** las 8 externas, probables con un Odoo de mentira), `engine/precios_rapidos.py` (la mina 1 del inventario, y sus ceros ahora quedan contados) y `engine/conciliacion.py` (las dos referencias de un residual, que estaba definida adentro de un `for`, **y** el reparto FIFO de un pago). y `engine/saldos.py` (los cuatro saldos, **el que consume el FIFO**, con sus cuatro trampas fijadas de a una, más la valuación de notas de crédito). y `engine/reversadas.py` (las dos lecturas de una factura anulada, con el defecto medido en 17 órdenes y 4.489,12 USD). **292 tests nuevos**; `app.py` bajó ~790 líneas. Es la tarea más larga del plan y se hace por pedazos. |
